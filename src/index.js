...
app.post("/api/user/refresh", jsonParser, async (_req, res) => {
 try {
  const { refresh_token, user_id } = _req.body;
  if (!refresh_token) {
   unauthorized(res);
   return;
  }
...
app.patch("/api/user/:id", jsonParser, async (_req, res) => {
 try {
  ...
  const { first_name, middle_name, last_name, user_email, user_type_id } =
   _req.body;
  const queryString =
   "UPDATE lirra.user SET first_name = $1, middle_name = $2, last_name = $3, user_email = $4, user_type_id = $5 WHERE user_id = $6;";
  const params = [
   first_name,
   middle_name,
   last_name,
...
app.post("/api/course", jsonParser, async (_req, res) => {
 try {
  ...
  const queryString =
   "INSERT INTO lirra.course(course_id, course_name) VALUES($1, $2);";
...
app.patch("/api/course/:id", jsonParser, async (_req, res) => {
 try {
  ...
  const { course_name } = _req.body;
  const queryString =
   "UPDATE lirra.course SET course_name = $1 WHERE course_id = $2;";
...
app.get("/api/library-resource/:id", async (_req, res) => {
 try {
  ...
  const id = _req.params.id;
  const queryString =
   "SELECT * FROM lirra.library_resource WHERE library_resource_id = $1;";
  const params = [id];
  const result = await apiCall(params, queryString, res);
  if (result) {
   success(result.rows, res);
...
app.delete("/api/library-resource/:id", async (_req, res) => {
 try {
  ...
  const id = _req.params.id;
  const queryString =
   "DELETE FROM lirra.library_resource WHERE library_resource_id = $1;";
  const params = [id];
  const result = await apiCall(params, queryString, res);
  if (result) {
   success("success", res);
...
app.post("/api/recommendation", jsonParser, async (_req, res) => {
 try {
  ...
  const { user_id, library_resource_id } = _req.body;
  const queryString = `
...
app.get("/api/recommendation/:id", async (_req, res) => {
 try {
  ...
  const id = _req.params.id;
  const queryString =
   "SELECT * FROM lirra.recommendation WHERE recommendation_id = $1;";
  const params = [id];
  const result = await apiCall(params, queryString, res);
  if (result) {
   success(result.rows, res);
...
app.patch("/api/recommendation/:id", jsonParser, async (_req, res) => {
 try {
  ...
  const { user_id, library_resource_id } = _req.body;
  const queryString =
   "UPDATE lirra.recommendation SET user_id = $1, library_resource_id = $2 WHERE recommendation_id = $3;";
